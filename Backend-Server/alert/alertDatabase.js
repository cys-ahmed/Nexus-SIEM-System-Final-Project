...
async function saveAlert(alertData) {
    ...
    try {
        ...
        const insertQuery = `
      INSERT INTO "Alert" ("Timestamp", "Severity", "Status", "Title", "Description", "Source", "Event_Id")
...
async function getAlerts(options = {}) {
    try {
        ...
        let query = `
      SELECT "ALERT_ID", "Event_Id", "Timestamp", CAST("Severity" AS INTEGER) as "Severity", "Status", "Title", "Description", "Source", "Stage_Checks", "Review_Notes"
      FROM "Alert"
    `;

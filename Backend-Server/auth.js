...
async function authRoutes(fastify, options) {
    ...
    try {
      ...
      try {
        ...
        return reply.send({
          ...
          message: 'User created successfully with default password "NewUser"'
        });
  ...
  fastify.post('/forgot-password', async (request, reply) => {
    ...
    try {
      ...
      const frontendUrl = clientFrontendUrl || process.env.FRONTEND_URL || 'http://localhost:8080';

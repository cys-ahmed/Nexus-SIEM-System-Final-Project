...
export default function AdminIncidents() {
  ...
  const fetchIncidents = React.useCallback(async (pageNum = 0, isLoadMore = false) => {
    ...
    try {
      const { incidents: newActiveIncidents, hasMore: moreAvailable } = await fetchAllIncidents(pageNum, 50);
      ...
      if (isLoadMore) {
        ...
        const uniqueMap = new Map<string, Incident>();
        newActiveIncidents.forEach(item => uniqueMap.set(item.id, item));
        setActiveIncidents(Array.from(uniqueMap.values()));
      }
      ...
      if (id.startsWith('log-')) {
        ...
        if (data.success) {
          if (incident.severity === 'critical' || incident.severity === 'high') {
            console.log("Detection resolved:", incident.title);
          }
          toast({
                  ...
                  {filteredActiveIncidents.map((incident) => (
                    <Dialog key={incident.id}>
                      <DialogTrigger asChild>
                        <div>
                          <AlertCard {...incident} time={formatTimestamp(incident.time)} recovery={incidentRecovery[incident.id] ?? 0} onRemove={() => resolveIncident(incident.id)} />
                        </div>
                      </DialogTrigger>
                      <DialogContent>
                        <DialogHeader>
